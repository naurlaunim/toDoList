...
class ToDoList extends Component {
	...
	handleDone = (event) => {
		...
		const newTask = Object.assign({}, task, {date: date});
		const prevTask = this.state.history.find(t => t.text === name);
		// const history = (prevTas)
		if (prevTask) {
			console.log(1)
			console.log(this.state.history)
			console.log(this.state.history.filter(task => task.text !== name))
			console.log(newTask)
			console.log(this.state.history.filter(task => task.text !== name).concat(newTask))
			this.setState({
				history: this.state.history.filter(task => task.text !== name).concat(newTask)
			})
			console.log(this.state.history)
		}
		this.setState({
					...
					<div className='taskslist'>
						...
						}
						{
							this.getList().map(task =>
								<div className='row' key={!this.state.showHistory ? task.text.toString() : task.date} id={task.text.toString()}>
									{!this.state.showHistory
